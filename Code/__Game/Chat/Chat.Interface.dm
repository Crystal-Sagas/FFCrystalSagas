...
/mob/proc
	 ...
	 * Uses default.browser1 for styled HTML chat
	 */
	initBrowseChat()
		...
		// The browser1 element is in the "default" window
		client << output(html, "default.browser1")

		SystemLog("CHAT", "DEBUG", "initBrowseChat output sent to default.browser1")
		...
		spawn(15)
			...
			if(isAdmin())
				client << output("1", "default.browser1:setAdminMode")
				// Admins should always see rank chat tab
				client << output("1", "default.browser1:setRankMode")
			else
				client << output("0", "default.browser1:setAdminMode")
	...
	closeBrowseChat()
		...
		client << output(null, "default.browser1")
		chat_window_open = FALSE
