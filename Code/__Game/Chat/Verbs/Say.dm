...
/mob/proc/processQuotedSay(msg, quote_player, quote_id, quote_text)
	...
	if(InFusion)
		for(var/mob/m in world)
			if(m.FusionKey2 == usr.FusionKey2)
				Sender = m
				break
	...
	world.log << "DEBUG processQuotedSay: quote_player=[quote_player], quote_id=[quote_id], quote_text=[quote_text]"
...
/mob/verb/Say(msg as text)
	...
	// Handle fusion (sender may be the fusion partner)
	var/mob/Sender = src
	if(InFusion)
		for(var/mob/m in world)
			if(m.FusionKey2 == usr.FusionKey2)
...
/mob/proc/handleAdminModeViewers(mob/Sender, SQ, msg)
	for(var/mob/Admin_Mode/M in view(usr))
		...
		for(var/mob/player/P in world)
			if(P.ckey != M.controllerKey) continue
			if(!P.client || !P.client.holder) continue
			
			// Build structured data with admin mode flag
			var/list/msg_data = list(
				"ic",                    // channel
				"[Sender]",              // speaker
				msg,                     // message untranslated for admin
				"",                      // no language indicator for admin mode
				SQ,                      // quote style
				usr.TextColor,           // color
				"1",                     // heard
				"adminmode"              // special flag
			)
			
			// Add admin link
			msg_data += "\ref[P.client.holder]"
			msg_data += "\ref[usr]"
			
			// Send to both IC and OOC channels
			if(P.chat_window_open)
				P.sendChatMessageFromList(msg_data)
			else
				if(!P.chat_message_buffer)
					P.chat_message_buffer = list()
				P.chat_message_buffer += list(list("channel" = "ic", "data" = msg_data))
...
/mob/proc/handleOOCMessage(list/recipients, SQ, msg)
	for(var/mob/player/M in recipients)
		...
		if(M.client && M.client.holder)
			msg_data += "\ref[M.client.holder]"    // admin_ref
...
/mob/proc/handleICMessage(list/recipients, mob/Sender, SQ, msg, is_exclaim, quote_player = null, quote_id = null, quote_text = null)
	...
	world.log << "DEBUG handleICMessage: quote_player=[quote_player], quote_id=[quote_id], quote_text=[quote_text]"
	
	// Two paths: button quote (params provided) or @mention quote (parse from message)
	world.log << "DEBUG handleICMessage: quote_player=[quote_player], quote_id=[quote_id], quote_text=[quote_text ? copytext(quote_text, 1, 50) : "null"]"
	if(quote_player && quote_text)
		...
		world.log << "DEBUG: Button quote HTML generated, length=[length(quote_html_override)]"
		world.log << "DEBUG: HTML preview=[copytext(quote_html_override, 1, 150)]"
	...
	for(var/mob/player/M in recipients)
		...
		// Add admin link if recipient is admin
		if(M.client && M.client.holder)
			msg_data += "\ref[M.client.holder]"    // 8: admin_ref
		...
		if(M.Observer)
			handleObserverMessage(M, Sender, SQ, display_msg)
	
	// Add to quote history for future references use original untranslated message
	addToQuoteHistory(global_message_id, usr.name, original_msg, "ic")
...
/mob/proc/handleObserverMessage(mob/observer, mob/Sender, SQ, msg)
	...
	for(var/mob/player/S in Players)
		if(S.key != observer.Observer) continue
		
		var/observer_content = usr.LanguageSay(msg, usr.lan, usr.lan.Mastery, S)
		var/sender_name = "[Sender]"
		var/language = usr.lan ? "[usr.lan]" : ""
		
		// Build structured data with observer flag
		var/list/msg_data = list(
			"ic",                    // channel
			sender_name,             // speaker
			observer_content,        // translated message
			language,                // language indicator
			SQ,                      // quote style
			usr.TextColor,           // color
			"1",                     // heard
			"observer"               // special flag for observer mode
		)
		
		// Send via ter13 pattern
		if(!S.client) continue
		if(S.chat_window_open)
			S.sendChatMessageFromList(msg_data)
		else
			if(!S.chat_message_buffer)
				S.chat_message_buffer = list()
			S.chat_message_buffer += list(list("channel" = "ic", "data" = msg_data))
