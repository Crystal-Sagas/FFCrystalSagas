...
/client/proc
	...
	sendToBrowseChat(list/params) as num
		...
		world.log << "DEBUG sendToBrowseChat: Sending payload '[copytext(param_string, 1, 100)]'"
		src << output(param_string, "chatwindow.browser1:onMessage")
...
/mob/proc
	...
	sendChatMessage(channel = "all", speaker = "", message = "", language = "", quote_style = "", color = "", heard = "1", flag1 = "", flag2 = "", timestamp = "", message_id = "", alignment = "", badges = "", speaker_color = "", quote_html = "", metadata = "")
		world.log << "DEBUG sendChatMessage called: channel=[channel], speaker=[speaker], msg=[copytext(message, 1, 30)]..."
		
		if(!client)
			world.log << "DEBUG sendChatMessage: No client, message dropped"
			return FALSE
		if(!chat_window_open)
			world.log << "DEBUG sendChatMessage: Window not open, message dropped"
			return FALSE
		...
		world.log << "DEBUG sendChatMessage: Calling client.sendToBrowseChat"
