...
/datum/ChatMessageBuilder
	...
	proc/withAdminControls(admin_ref, sender_ref) as /datum/ChatMessageBuilder
		message.is_admin = TRUE
		message.admin_ref = admin_ref
...
/datum/RecipientContext
	...
	proc/getSenderName()
		...
		// Unknown contact - show signature
	...
	proc/canHear() as num
		...
		// Critical hearing disability
		if(recipient.Critical_Hearing)
			if(is_exclaim && prob(HEARING_CRITICAL_EXCLAIM_CHANCE))
				return 0
			if(!is_exclaim && prob(HEARING_CRITICAL_NORMAL_CHANCE))
				return 0
	...
	proc/getProcessedContent()
		...
		var/translated_msg = sender.LanguageSay(message, sender.lan, sender.lan.Mastery, recipient)
	...
	proc/buildMessage()
		...
		if(recipient.client && recipient.client.holder)
			builder.withAdminControls(recipient.client.holder, sender)
