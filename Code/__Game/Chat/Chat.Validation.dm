...
/datum/MessageValidator
	...
	proc/validate(message, mob/sender) as num
		...
		// Content validation - check if only whitespace
		var/test_msg = replacetext(message, " ", "")
		if(!length(test_msg))
...
/datum/ChatProfiler
	...
	proc/endProfile(operation_name)
		...
		var/list/op_times = operation_times[operation_name]
		op_times += duration
...
/**
 * Check if OOC chat is allowed in current context
 * Returns TRUE if OOC is permitted
 * NOTE: Crystal Sagas allows OOC everywhere by default
 *       This can be extended for specific area restrictions
 */
/mob/proc/CanOOC() as num
	// Allow OOC everywhere in Crystal Sagas
	// Can be extended to check area restrictions if needed
	return TRUE
