...
/**
 * Spawn test materials for testing crafting
 */
/mob/proc/spawnTestMaterials()
	src << system_chat("<font color='#8888FF'>Spawning test materials...</font>")

	// Spawn some metals
	var/obj/item/material/ore/Iron/iron1 = new()
	iron1.loc = src
	iron1.amount = 5
	iron1.updateStackDisplay()

	var/obj/item/material/ore/Steel/steel1 = new()
	steel1.loc = src
	steel1.amount = 3
	steel1.updateStackDisplay()

	// Spawn some wood
	var/obj/item/material/synthesis/Wood/wood1 = new()
	wood1.loc = src
	wood1.amount = 5
	wood1.updateStackDisplay()

	// Spawn leather
	var/obj/item/material/synthesis/Leather/leather1 = new()
	leather1.loc = src
	leather1.amount = 3
	leather1.updateStackDisplay()
 ...
 */
/mob/proc/learnAllProfessions()
	src << system_chat("<font color='#8888FF'>Learning all professions...</font>")

	learnProfession(CRAFT_PROF_WEAPONSMITH)
	learnProfession(CRAFT_PROF_ARMORSMITH)
	learnProfession(CRAFT_PROF_CARPENTER)
	learnProfession(CRAFT_PROF_TAILOR)
	learnProfession(CRAFT_PROF_LEATHERWORKER)
	learnProfession(CRAFT_PROF_ALCHEMIST)
	learnProfession(CRAFT_PROF_COOK)

	src << system_chat("<font color='#00FF00'>Learned all crafting professions!</font>")
