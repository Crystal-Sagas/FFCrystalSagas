...
hud/widget/slider
	...
	var/tmp
		...
		vector/handle_size	//the size of the handle widget
		vector/track_size	//the size of the track's draggable area

		vector/value		//the current value of the slider (can be set via boilerplate)
	...
	New(loc,hud/hud)
		handle_size = vector(32,32)
		track_size = vector(96,32)
		value = vector(0,0)
		..()
