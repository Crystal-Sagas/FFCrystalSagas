...
obj
	...
	FATEGiver
		...
		DblClick()
			...
			// locally cache party
			var/datum/party/accepting_party = usr.active_party
			// check they're not already on a fate
			if(accepting_party.has_active_fate())
				alert(usr,"Your party is already on a FATE mission!")
				usr.usingfate=0
				return
			// check cooldown
			var/mob/on_cooldown
			if((on_cooldown = accepting_party.first_member_has_fate_cooldown()))
				alert(usr,"[on_cooldown.name] is on FATE cooldown, and thus the party cannot accept a FATE!")
				usr.usingfate=0
				return
			var/choice = alert(usr, "Would you like to take a FATE quest for your party?", "FATE Initiation", "Yes", "No")
			if(choice == "No")
				usr.usingfate=0
				return
			#warn impl below
