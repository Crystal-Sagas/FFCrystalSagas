...
/obj/node/GatheringMoogle/Click()
	...
	for(var/obj/item/material/ore/Platinum/d in usr.contents)//These are to prevent people from getting these higher tier, rare items from gathering moogles. Prob can be coded better but this is functional at least.
		d.amount-=1
	for(var/obj/item/material/ore/Orichalcum/e in usr.contents)
		e.amount-=1
...
/obj/node/Minenode/Click()
	...
	var/roll2
	roll=rand(1,100)
	if(usr.check_perk("Seasoned Miner"))
		roll+=10
	if(roll<=39)
...
/obj/node/Herbnode/Click()
	...
	var/roll
	roll=rand(1,100)
	if(usr.check_perk("Seasoned Gatherer"))
		roll+=10
	if(roll>1&&roll<21)
...
/obj/node/Hunternode/Click()
	...
	var/roll
	roll=rand(1,100)
	if(usr.check_perk("Seasoned Hunter"))
		roll+=10
	if(roll<11)
